...
interface HistoryPanelProps {
  ...
  onSelectInvoice?: (name: string, items: CartLineItem[]) => void;
...
const HistoryPanel: React.FC<HistoryPanelProps> = ({
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (switcherMode === 'horizontal') {
        if (e.key === 'ArrowRight') selectInvoice(Math.min(activeIdx + 1, cards.length - 1));
        if (e.key === 'ArrowLeft') selectInvoice(Math.max(activeIdx - 1, 0));
      }
      if (switcherMode === 'vertical') {
        if (e.key === 'ArrowDown') selectInvoice(Math.min(activeIdx + 1, cards.length - 1));
        if (e.key === 'ArrowUp') selectInvoice(Math.max(activeIdx - 1, 0));
      }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [isOpen, handleClose, cards.length, switcherMode, activeIdx, selectInvoice, gridZoomed]);
  ...
  const onPointerUp = useCallback(() => {
    ...
    dragAxis.current = 'none';
    setDragDelta(0);
  }, [isDragging, dragDelta, cards.length, switcherMode, activeIdx, selectInvoice]);
  ...
  return (
    <div
      ...
      className={`fixed inset-0 z-120 transition-all duration-280 ${
        switcherMode === 'grid'
          ? ''
          : 'flex items-end sm:items-center justify-center p-4 pb-6 sm:pb-4'
    ...
    >
      ...
      {switcherMode === 'grid' ? (
        <>
          ...
          {gridZoomed && cards[activeIdx] && (
            <div
              className={`absolute inset-0 z-20 flex items-center justify-center p-4 pb-6 sm:pb-4 pointer-events-none transition-all duration-500 ${
                isOpen ? 'opacity-100' : 'opacity-0'
              }`}
            >
              <div
      ...
      <div
        ref={stageRef}
        className={`relative ${receiptStageClass} select-none transition-all duration-500 overflow-visible ${
      ...
      >
        {switcherMode === 'vertical' ? (
          ...
          cards.map((card, idx) => {
            const { translateX, translateY, scale, opacity, blurPx, zIndex, transformOrigin, isActive } = getCardStyle(idx);
